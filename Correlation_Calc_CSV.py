...
for team in teams:
    ...
    print("Below is the matrix for :", team)
    print(team)
    print(best_scores.corr())
    best_scores.corr().to_csv(team+"_corr.csv")
'''
with open("FD-salaries.csv") as csvdoc:
    csv_reader = csv.reader(csvdoc)
    for team in teams:
        print(team)
        for year in [2014,2015,2016,2017,2018]:
            print(year)
            for week in range (0,18):
                if Week[week] == week:
                    print("this week works")
                    data = [row for row in csv.reader(csvdoc)]
                else:
                    print("This is a bye week")
'''
